...
export function CollectionsManager({ storeId }: CollectionsManagerProps) {
  ...
  const { executeAsync: deleteCollection } = useAction(deleteCollectionAction)
  ...
  async function loadCollections() {
    setIsLoading(true)
    const result = await getCollections({ storeId, includeInactive: true })
    if (result?.data) {
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800">
            <IconFolders className="h-6 w-6 text-slate-400" />
          </div>
          <p className="mt-3 text-sm font-medium text-slate-600 dark:text-slate-300">
            Nenhuma coleção cadastrada
          </p>
          <p className="mt-1 text-xs text-slate-500">
            Adicione coleções para organizar seus produtos
          </p>
        </div>
      )}
