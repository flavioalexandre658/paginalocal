...
export default async function DashboardLayout({ children, params }: LayoutProps) {
  ...
  let storeData: { id: string; name: string; faviconUrl: string | null; useV2Renderer: boolean }[] = []

  if (isAdmin) {
    storeData = await db
      .select({ id: store.id, name: store.name, faviconUrl: store.faviconUrl, useV2Renderer: store.useV2Renderer })
      .from(store)
